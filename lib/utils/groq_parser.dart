...
class GroqParser {
  ...
  static GroqMessage groqMessageFromJson(Map<String, dynamic> json) {
    return GroqMessage(
      ...
      role: GroqMessageRoleParser.tryParse(json['role'] as String) ?? GroqMessageRole.user,
    );
  }

  ///Parses the rate limit information from the headers
  static GroqRateLimitInformation rateLimitInformationFromHeaders(Map<String, String> headers) {
  ...
  static GroqResponse groqResponseFromJson(Map<String, dynamic> json) {
    return GroqResponse(
      ...
      choices: (json['choices'] as List)
          .map(
            (item) => GroqParser.groqChoiceFromJson(item as Map<String, dynamic>),
          )
          .toList(),
      createdAt: DateTime.fromMillisecondsSinceEpoch(json['created'] as int, isUtc: true),
    );
  }
  ...
  static GroqUsage groqUsageFromChatJson(Map<String, dynamic> json) {
    return GroqUsage(
      ...
      promptTime: Duration(milliseconds: ((json['prompt_time'] as double) * 1000).toInt()),
      completionTime: Duration(milliseconds: ((json['completion_time'] as double) * 1000).toInt()),
    );
  }
  ...
  static GroqUsage groqUsageFromAudioJson(Map<String, dynamic> json) {
    return GroqUsage(
      ...
      completionTime: Duration(milliseconds: ((json['total_time'] as double) * 1000).toInt()),
    );
  }

  static GroqLlamaGuardCategory? groqLlamaGuardCategoryFromString(String category) {
    switch (category) {
      case 'S1':
